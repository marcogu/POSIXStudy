...
Storing the request number of seconds would not be enough, 
since the "alarm thread" cannot tell how long it has been on the list.
*/
	...
	*/
	while(1){
		...
		*/
		...
		// check status value is right (be zero)
		if (sleep_time > 0)
		{
			sleep(sleep_time);
		}
...
int main(int argc, char const *argv[])
{
	...
	while(1)
	{
		...
		*/
		...
		{
			status = pthread_mutex_lock(&alarm_mutex);
			// check status, assert status is zero.
